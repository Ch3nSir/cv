...
def main():
    ...
    for idx in range(1):
        ...
        Y = np.transpose(img_L, (2, 0, 1)).reshape([channel, hight * width])
        ...
        for i in range(L):
            U = U1[:, i]                        #U是光谱表示，L从列里选取，可调
            E = np.matmul(U.T,Y)               #E=Ut*Y 计算空间表示系数 E，通过将 Y 投影到光谱基 U 上。
            ...
            #重新拼接E
            # ------------------------------------
            img_cat = torch.cat((img_cat, img_E), dim=0) if i > 0 else img_E
            print(img_cat.shape)
        img_E = img_cat
        print(img_E.shape)
        ...
        img_E_tensor = util.single2tensor4(util.uint2single(img_E)).to(device)
        img_clean_tensor = util.single2tensor4(util.uint2single(img_clean_dc)).to(device)
    
        loss = criterion(img_E_tensor, img_clean_tensor)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    save_model_path = os.path.join(model_pool, model_name + '_trained.pth')
    torch.save(model.state_dict(), save_model_path)
    logger.info('Saved trained model to {:s}'.format(save_model_path))
